...
from parapy.lib.avl.readers import read_surface_forces
from parapy.lib.avl.readers import read_element_forces
from parapy.lib.avl.readers import read_strip_forces
...
class Aircraft(Base):
    ...
    read = ReadInput(file_name='constants.csv')
    ...
    #: :type: float or str
    sweep_angle = Input(variables["sweep_angle"])  # Overwrites default function if ~= 'NaN'

    #: wing taper ratio []
    #: :type: float or str
    taper_ratio = Input(variables["taper_ratio"])  # Overwrites the default function if ~= 'NaN'

    #: wing dihedral angle [degrees]
    #: :type: float or str
    dihedral_angle = Input(variables["dihedral_angle"])  # Overwrites the default function if ~= 'NaN'

    #: jet cruise speed [mach]
    #: :type: float
    m_cruise = Input(variables["m_cruise"])  # Used to calculate the default sweep angle
    ...
    #: :type: float
    w_loc_perc = Input(variables["w_loc_perc"])
